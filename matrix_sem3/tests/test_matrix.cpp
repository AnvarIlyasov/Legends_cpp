...
TEST_CASE("checkSquareMatrix returns true for square and false for non-square",
          "[square]") {
  std::vector<std::vector<int>> square{{1, 2}, {3, 4}};
  std::vector<std::vector<int>> non_square{{1, 2, 3}, {4, 5, 6}};

  const bool square_result = checkSquareMatrix(square);
  const bool non_square_result = checkSquareMatrix(non_square);

  REQUIRE(square_result);
  REQUIRE_FALSE(non_square_result);
}
